...
function FitnessCertificate() {
    ...
    const fetchStaff = async () => {
        setLoading(true)
        try {
            const res = await getSecureApiData(`api/certificate/fitness?page=${page}&limit=${limit}&search=${search}`);
    ...
    const downloadDoctors = () => {
        ...
        const data = certList.map((doc, index) => ({
            ...
            Examin: new Date(doc?.examinDate)?.toLocaleDateString('en-GB') || 0,
            Effective:new Date(doc?.effectiveDate)?.toLocaleDateString('en-GB') || 0,
        }));

        const worksheet = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();

        XLSX.utils.book_append_sheet(workbook, worksheet, "FitnessCertificate");
        ...
        saveAs(fileData, "FitnessCertificate_List.xlsx");
    };
    return (
        <>
            {loading ? <Loader />
                        ...
                        <div className="d-flex align-items-center justify-content-between">
                            ...
                            <div className="add-nw-bx">
                                {/* {(isOwner || permissions?.doctors?.add) && ( */}
                                    <NavLink to="/fitness-form" className="add-nw-btn nw-thm-btn">
                                                ...
                                                {certList?.length === 0 ? (
                                                    ...
                                                    certList?.map((doc, index) => (
                                                        <tr key={doc._id}>
                                                            ...
                                                            <td>{doc?.examinDate ? new Date(doc?.examinDate)?.toLocaleDateString('en-GB') : '-'}</td>
                                                            <td>{doc?.effectiveDate ? new Date(doc?.effectiveDate)?.toLocaleDateString('en-GB') : '-'}</td>
                                                            ...
                                                            <td>
                                                                ...
                                                                <Link
                                                                    to={`/certificate/fitness/${doc._id}`}
