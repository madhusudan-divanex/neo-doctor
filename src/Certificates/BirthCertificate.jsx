...
function BirthCertificate() {
    ...
    const fetchStaff = async () => {
        setLoading(true)
        try {
            const res = await getSecureApiData(`api/certificate/birth?page=${page}&limit=${limit}&search=${search}&date=${date}`);
    ...
    const downloadDoctors = () => {
        ...
        const data = certList.map((doc, index) => ({
            ...
            Mother: doc?.motherName || 0,
            Date: doc?.dateOfBirth || "",
            Delivery: doc?.deliveryType || "",
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "BirthCertificate");
        ...
        saveAs(fileData, "BirthCertificate_List.xlsx");
    };
    return (
        <>
            {loading ? <Loader />
                : <div className="main-content flex-grow-1 p-3 overflow-auto">
                    <div className="row ">
                        <div className="d-flex align-items-center justify-content-between">
                            <div>
                                <h3 className="innr-title mb-2 gradient-text">Birth Certificates</h3>
                            ...
                            <div className="add-nw-bx">
                                {/* {(isOwner || permissions?.doctors?.add) && ( */}
                                    <NavLink to="/birth-form" className="add-nw-btn nw-thm-btn">
                                                ...
                                                {certList?.length === 0 ? (
                                                    ...
                                                    certList?.map((doc, index) => (
                                                        <tr key={doc._id}>
                                                                ...
                                                                <div className="admin-table-bx">
                                                                    <div className="admin-table-sub-bx">
                                                                        ...
                                                                        <div className="admin-table-sub-details">
                                                                            <h6>{doc?.childName}</h6>
                                                                            <h6>Father:{doc?.fatherName}</h6>
                                                                            <h6>Mother:{doc?.motherName}</h6>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            ...
                                                            <td>{new Date(doc?.dateOfBirth)?.toLocaleDateString('en-GB')}</td>
                                                            <td>{doc?.gender}</td>
